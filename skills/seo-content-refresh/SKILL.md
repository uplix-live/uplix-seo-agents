...
### Règles de rédaction GEO (citabilité IA) — obligatoires

À appliquer en plus de la calibration ci-dessus, pour rendre la page citable par les moteurs IA (AI Overviews, ChatGPT search, Perplexity) :

| Règle GEO | Cible |
|---|---|
| **Content Capsule** | **60–70 % des H2 formulés en questions** ; la **1re phrase de chaque section = réponse auto-suffisante** (extractible en passage), **sans pronom de reprise** (« Il/Elle/Cela/Ce/Cette… ») |
| **Three Kings** | KW principal présent dans le **title** + le **1er paragraphe** + **≥ 2 H2** |
| **Information Gain** | ≥ 1 section qui apporte ce qui n'est PAS dans le top 10 : donnée originale, angle contre-intuitif, synthèse novatrice, expérience first-hand réelle |
| **Entités saillantes** | 5–10 entités nommées récurrentes du top 5 placées naturellement |
| **Ancres** | **1–3 mots**, descriptives — jamais « ici / cliquez ici / en savoir plus » ; liens **inline dans le corps** (pas de bibliographie en bas) |
| **Anti-fabrication** | si pas d'expérience réelle disponible (research-only mode) : **interdiction d'inventer** du first-person ou des anecdotes — rester factuel et sourcé |
...
### 7a — Linter de contenu déterministe (sur le Markdown source)

Avant la gate des 14 marqueurs, passer le contenu rédigé (`01-nouveau-contenu.md`) dans le linter déterministe — il vérifie la **citabilité GEO** (Content Capsule, Three Kings, ancres 1–3 mots, auto-suffisance, phrases de remplissage, concurrents, word count, TL;DR) :

```bash
python scripts/lint_post.py 01-nouveau-contenu.md \
  --keyword "<KW principal>" \
  [--competitors "concurrent1.fr,concurrent2.com"] \
  [--target <mean_length concurrents>]
```

- **Exit code 2 (erreur bloquante : hiérarchie Hn cassée ou Three Kings KO)** → corriger AVANT de poursuivre. **0 erreur requis.**
- **Exit code 1 (warnings : capsule hors plage, ancres, remplissage…)** → revoir ; non bloquant mais à minimiser.
- Ce contrôle est **complémentaire** des 14 marqueurs : le linter agit sur le Markdown (citabilité), `markers_check.py` sur le HTML WP-friendly (marqueurs de publication). **Les deux doivent passer avant un push.**

### 7b — Gate des 14 marqueurs (sur le HTML WP-friendly)
