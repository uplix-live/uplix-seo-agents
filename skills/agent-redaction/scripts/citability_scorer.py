...
Source: geo-seo-claude (MIT License)
Usage: python scripts/citability_scorer.py <url>
"""
...
def score_passage(text: str, heading: Optional[str] = None) -> dict:
    ...
    definition_patterns = [
        ...
        r"\ben\s+(?:simple|autres)\s+(?:termes|mots)\s*,",
        r"\best\s+(?:un|une|le|la)\s",
    ]
    ...
    if any(
        ...
        for p in [
            r"\b(?:is|are|was|were|est|sont|means?|refers?)\b",
            r"\d+%",
            r"\$[\d,]+",
            r"[\d,]+\s*€",
            r"\d+\s+(?:million|billion|thousand|millions|milliards)",
        ]
    ):
        abq_score += 15
    ...
    if re.search(
        r"(?:according to|research shows|studies? (?:show|indicate|suggest|found)|data (?:shows|indicates|suggests)|selon|d'après|une étude|des recherches)",
        text,
        re.IGNORECASE,
    ):
        abq_score += 10
    ...
    pronoun_count = len(
        re.findall(
            r"\b(?:it|they|them|their|this|that|these|those|he|she|his|her|il|elle|ils|elles|ce|cela|celui|celle)\b",
            text,
            re.IGNORECASE,
        )
    )
    ...
    if re.search(r"(?:first|second|third|finally|additionally|moreover|furthermore|premièrement|deuxièmement|enfin|de plus|par ailleurs)", text, re.IGNORECASE):
        sr_score += 4

    # Contains numbered items or bullet-like content
    if re.search(r"(?:\d+[\.\)]\s|\b(?:step|tip|point|étape|conseil)\s+\d+)", text, re.IGNORECASE):
        sr_score += 4
    ...
    # Dollar or Euro amounts
    dollar_count = len(re.findall(r"(?:\$[\d,]+(?:\.\d+)?(?:\s*(?:million|billion|M|B|K))?|[\d,]+\s*€)", text))
    sd_score += min(dollar_count * 3, 5)

    # Other numbers with context
    number_count = len(re.findall(r"\b\d+(?:,\d{3})*(?:\.\d+)?\s+(?:users|customers|pages|sites|companies|businesses|people|percent|times|x\b|utilisateurs|clients|entreprises)", text, re.IGNORECASE))
    ...
    # Named sources
    source_patterns = [
        r"(?:according to|per|from|by|selon|d'après)\s+[A-Z]",
        r"(?:Gartner|Forrester|McKinsey|Harvard|Stanford|MIT|Google|Microsoft|OpenAI|Anthropic|Ahrefs|Semrush)",
        r"\([A-Z][a-z]+(?:\s+\d{4})?\)",
    ]
    ...
    if re.search(
        r"(?:our (?:research|study|data|analysis|survey|findings)|we (?:found|discovered|analyzed|surveyed|measured)|notre (?:étude|analyse|recherche)|nous avons (?:trouvé|découvert|analysé))",
        text,
        re.IGNORECASE,
    ):
        us_score += 5
    ...
    if re.search(
        r"(?:case study|for example|for instance|in practice|real-world|hands-on|étude de cas|par exemple|en pratique|concrètement)",
        text,
        re.IGNORECASE,
    ):
        us_score += 3
    ...
    if re.search(r"(?:using|with|via|through|avec|grâce à|via)\s+[A-Z][a-z]+", text):
        us_score += 2
